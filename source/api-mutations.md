...
要读取您正在更改的 store 中的数据，请确保使用 [`DataProxy`][] 的诸如 [`readQuery`][] 和 [`readFragment`][] 方法。
...
您提供给该函数的第一个参数，是您的查询的旧数据对象的值。所以如果你的键是 `Comments`，那么第一个参数将是你的 `Comments` 查询返回的最后一个数据对象，或者是使用 `Comments` 查询的任意组件所使用的当前对象。
