...
上述突变将向 GitHunt 提交一个新的 GitHub 存储库，保存一个条目到数据库中。结果可能是：
...
当我们在Apollo中使用突变时，结果通常会[根据结果的 id](cache-updates.html＃dataIdFromObject) 自动整合到缓存中，而这又自动更新UI，所以我们经常不需要明确处理结果。为了让客户端正确地执行此操作，我们需要确保在结果中选择必要的字段。有一个好的策略可以是简单地请求可能受到突变影响的任何字段。或者，您可以使用[片段](fragments.html)共享查询和更新该查询的突变之间的字段。
